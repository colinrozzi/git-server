...
pub fn compress_zlib(data: &[u8]) -> Vec<u8> {
    ...
    encoder
        .write_all(data)
        .expect("Writing to ZlibEncoder should never fail");

    // Finish compression and return the compressed bytes
    encoder
        .finish()
        .expect("Finishing ZlibEncoder should never fail")
}
...
mod tests {
    ...
    fn test_compress_zlib_large_data() {
        ...
        let data =
            b"This is a test string that should compress well because it has repetitive content. "
                .repeat(100);
        let compressed = compress_zlib(&data);
    ...
    fn test_compression_performance() {
        ...
        assert!(
            compressed.len() < repetitive_data.len() / 10,
